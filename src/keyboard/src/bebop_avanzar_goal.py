...
class keyboard:
	def __init__(self):
		...
		#-----
		self.goal_topic = rospy.Subscriber('/camera/goal', Int8, self.flag7)
		self.distance_goal_topic = rospy.Subscriber('/camera/dist_goal', Float64, self.flag8)
	...
	def cmd_vel(self):
		...
		while not rospy.is_shutdown():
			while(self.goal==0):
				...
				if self.VALUE_OBJETOS_DISTANCES[0]>1.5 and self.VALUE_OBJETOS_DISTANCES[0]<999: #sino hay nada enfrente, adelante
					key='W'
					rospy.sleep(0.5)
				else: #si hay algo enfrente, verificar izquierdas
					if self.VALUE_OBJETOS_DISTANCES[1]<1.5: 
						if self.VALUE_OBJETOS_DISTANCES[2]<1.5: #si hay algo izquierda y derecha, subir
							key='Q'
							rospy.sleep(1)
						else:
							key='D' #si hay algo en izquierda y no derecha, derecha
							rospy.sleep(1)
					else: #sino hay algo en izq,izq
						key='A'
						rospy.sleep(1)
				print("distance  ",self.VALUE_OBJETOS_DISTANCES)
				...
				rospy.sleep(5)
			if(self.goal==1):
				#si ya ha encontrado el objetivo
				print("Distance to the goal", self.dist_goal)
				key='SPACE'
				self.key_commands(key)

	def key_commands(self,key):
		if key == 'RIGHT':
