...
class keyboard:
	def __init__(self):
		...
		self.rgbImg = rospy.Subscriber('/bebop2/front_camera/rgb/image_raw', Image, self.guardar_img, queue_size=1, buff_size=2**24)
		self.init_msg = """		-------------------------------
		...
		self.cmd_vel()
		self.scale=[]
	def guardar_img(self,data):
		frame = self.bridge.imgmsg_to_cv2(data, "bgr8")
		self.scale = cv2.resize(frame, (520, 440))
		...
		while not rospy.is_shutdown():
			while(bandera==0):
				...
				if key!=keys.T:
					...
					cv2.imwrite('/home/nilda/Documentos/img/4/' + str(len(self.archivo)) + '.jpg',self.scale)
