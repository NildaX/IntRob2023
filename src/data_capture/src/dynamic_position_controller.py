...
class bebop_control:
	def __init__(self):
		...
		#self.Gazebo = rospy.Subscriber('/gazebo/model_states', ModelStates, self.ORBSLAM_pose)
		self.ORBpose = rospy.Subscriber('/ORB2/pose', PoseStamped, self.ORBSLAM_pose)
	...
	def ORBSLAM_pose(self,data):
		...
		self.qx = model_states.pose[index].orientation.x;
		self.qy = model_states.pose[index].orientation.y;
		self.qz = model_states.pose[index].orientation.z;
		self.qw = model_states.pose[index].orientation.w;
		'''
		...
		self.display_path(self.x, self.y, self.z, self.qx, self.qy, self.qz, self.qw)
