...
class image_viewer:
	def __init__(self):
		...
		self.pub_objects_distances_fake = rospy.Publisher('/camera/objects_distances_fake', Float32MultiArray, queue_size=10)
		...
		self.cont =   0
		self.data = { 'seccion_0': [0],'seccion_1': [0],'seccion_2': [0],'seccion_3': [0],'seccion_4': [0], 'accion': ['W'],'seccionf_0': [0],'seccionf_1': [0],'seccionf_2': [0],'seccionf_3': [0],'seccionf_4': [0], 'accionf': ['W']}
		self.df = pd.DataFrame(self.data)
		self.distancias_objetos=[]
		self.distancias_objetos_fake=[]
	...
	def callback(self,data):
		...
		self.distancias_objetos=distancias_objetos
		...
		wait_predict=self.predict()
		wait_fake=self.publicar_()
		if (self.cont % 2 ==0):
			action_f='w'
			if self.distancias_objetos[0]>1.5 and self.distancias_objetos[0]<999: #sino hay nada enfrente, adelante
					action='W'
			else: #si hay algo enfrente, verificar izquierdas
					if self.distancias_objetos[1]<1.5: 
						if self.distancias_objetos[2]<1.5: #si hay algo izquierda y derecha, subir
							action='Q'
						else:
							action='D' #si hay algo en izquierda y no derecha, derecha
					else: #sino hay algo en izq,izq
						action='A'

			if self.distancias_objetos_fake[0]>1.5 and self.distancias_objetos_fake[0]<999: #sino hay nada enfrente, adelante
					action_f='W'
			else: #si hay algo enfrente, verificar izquierdas
					if self.distancias_objetos_fake[1]<1.5: 
						if self.distancias_objetos_fake[2]<1.5: #si hay algo izquierda y derecha, subir
							action_f='Q'
						else:
							action_f='D' #si hay algo en izquierda y no derecha, derecha
					else: #sino hay algo en izq,izq
						action_f='A'

			
			new_data = { 'seccion_0': self.distancias_objetos[0],'seccion_1': self.distancias_objetos[1],'seccion_2': self.distancias_objetos[2],'seccion_3': self.distancias_objetos[3],'seccion_4': self.distancias_objetos[4], 'accion': action,'seccionf_0': self.distancias_objetos_fake[0],'seccionf_1': self.distancias_objetos_fake[1],'seccionf_2': self.distancias_objetos_fake[2],'seccionf_3': self.distancias_objetos_fake[3],'seccionf_4': self.distancias_objetos_fake[4], 'accionf': action_f}
			self.df = (self.df).append(new_data, ignore_index=True)
		if (self.cont > 55):#10000):
			excel_file_path = '/home/nilda/data_update.xlsx'
	...
	def publicar_(self):
		...
		print("objetos fake",distancias_objetos)
		msg = Float32MultiArray()
		msg.data = distancias_objetos
		#msg.dist = distancias_objetos
		self.pub_objects_distances_fake.publish(msg)
		self.distancias_objetos_fake=distancias_objetos
		return 1
