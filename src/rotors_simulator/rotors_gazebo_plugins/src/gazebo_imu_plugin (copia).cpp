...
void GazeboImuPlugin::AddNoise(Eigen::Vector3d* linear_acceleration,
  ...
  GZ_ASSERT(dt > 0.0, "Change in time must be greater than 0 aaaa.");
...
void GazeboImuPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  if (dt <= 0.0) {
    // Reset last_time_ to current time
    last_time_ = current_time;
    // Recalculate dt
    dt = std::numeric_limits<double>::epsilon();//dt = 0.0; // Reset dt to 0.0 or any other appropriate value
    // Output a warning or handle the situation appropriately
    gzmsg << "Simulation time reset detected. Resetting last_time_ to current time." << std::endl;
}
  last_time_ = current_time;
